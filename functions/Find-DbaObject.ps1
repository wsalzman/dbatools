...
.EXAMPLE
Find-DbaObject -SqlInstance DEV01 -Pattern ad\stephen

Searches user objects for owner ad\stephen

.EXAMPLE
Find-DbaObject -SqlInstance DEV01 -Verbose
    ...
    process {
        foreach ($Instance in $SqlInstance) {
            ...
            if (-not $pattern) {
                ...
                Write-Verbose "Gather data on credentials"
                $creds = $server.Credentials | Where-Object { $_.Name -match $pattern }
                Write-Verbose "Gather data on proxy accounts"
                $proxies = $server.JobServer.ProxyAccounts | Where-Object { $_.Name -match $pattern }
                Write-Verbose "Gather data on endpoints"
                $endPoints = $server.Endpoints | Where-Object { $_.Name -match $pattern }
                Write-Verbose "Gather data on Agent Jobs ownership"
            ...
            if (-not $pattern) {
                ...
                foreach ($db in $server.Databases | Where-Object { $_.Name -match $pattern }) {
            ...
            ## tables
            if (-not $pattern) {
                foreach ($tbl in $server.Databases.Tables | Where-Object { $_.Owner -ne $saname }) {
                    Write-Verbose "checking if $tbl is owned "
                    ...
                    [PSCustomObject]@{
                        ...
                        Owner           = $tbl.Owner
                        Name            = $tbl.Name
                        Parent          = $tbl.Parent.Name
                    }
                }
            }
            else {
                foreach ($tbl in $server.Databases | Where-Object { $_.Name -match $pattern }) {
            ...
            if (-not $pattern) {
                ...
                foreach ($role in $server.Roles | Where-Object { $_.Name -match $pattern }) {
            ...
            foreach ($db in $server.Databases | Where-Object IsAccessible) {
                ...
                if (-not $pattern) {
                    $schemas = $db.Schemas | Where-Object { $_.IsSystemObject -eq 0 -and $sysSchemas -notcontains $_.Owner }
                }
                else {
                    $schemas = $db.Schemas | Where-Object { $_.IsSystemObject -eq 0 -and $_.Name -match $pattern -and $sysSchemas -notcontains $_.Owner }
                }
                ...
                ## database roles
                if (-not $pattern) {
                    $roles = $db.Roles | Where-Object { $_.IsSystemObject -eq 0 }
                }
                else {
                    $roles = $db.Roles | Where-Object { $_.IsSystemObject -eq 0 -and $_.Name -match $pattern }
                }
                ...
                ## assembly
                if (-not $pattern) {
                    $assemblies = $db.Assemblies | Where-Object { $_.IsSystemObject -eq 0 }
                }
                else {
                    $assemblies = $db.Assemblies | Where-Object { $_.IsSystemObject -eq 0 -and $_.Name -match $pattern }
                }
                ...
                ## synonyms
                if (-not $pattern) {
                    $synonyms = $db.Synonyms | Where-Object { $_.IsSystemObject -eq 0 }
                }
                else {
                    $synonyms = $db.Synonyms | Where-Object { $_.IsSystemObject -eq 0 -and $_.Name -match $pattern }
                }
